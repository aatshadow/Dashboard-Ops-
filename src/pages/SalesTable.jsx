...
export default function SalesTable() {
  const { getSalesWithNetCash, addSales, updateSale, deleteSale, getPaymentFees, getTeam, getProducts } = useClientData()
                  ...
                  <td>
                    {activeProducts.length > 0 ? (
                      <select value={editData.product} onChange={e => setEditData({...editData, product: e.target.value})}>
                        ...
                        {editData.product && !activeProducts.find(p => p.name === editData.product) && (
                          <option value={editData.product}>{editData.product}</option>
                        )}
                      </select>
                    ) : (
                      <input value={editData.product} onChange={e => setEditData({...editData, product: e.target.value})} />
                    )}
                  </td>
                  <td>
